...
const createStore = (createStore) => (reducer, initialState, enhancer) => {
  ...
  function registerReducers(reducers1, replaceReducers = false) {
    ...
    Object.keys(reducers1).forEach(key => {
      if (typeof key !== 'string') {
        throw new Error('Reducers paths must be strings')
      }
    ...
    Object.keys(reducers1).forEach(key => {
      ...
      if (foundPath.length) {
        throw new Error(`Reducer path ${key} already busy: ${foundPath.join(', ')}`)
      } else {
